...
# Build libcurl
###################
LIBCURLIMPERSONATE_LIBCURL=${LIBCURLIMPERSONATE_LIBCURL:-7.84.0}
LIBCURLIMPERSONATE_RELEASE=${LIBCURLIMPERSONATE_RELEASE:-0.5.4}
LIBCURLIMPERSONATE_DEST_FOLDER=$PREFIX_DIR/deps/libcurl-impersonate
echo "Building libcurl-impersonate v$LIBCURLIMPERSONATE_RELEASE"
./scripts/ci/build-libcurl-impersonate.sh $LIBCURLIMPERSONATE_RELEASE $LIBCURLIMPERSONATE_DEST_FOLDER $LIBCURLIMPERSONATE_LIBCURL || (echo "libcurl failed build log:" && cat_slower $LIBCURLIMPERSONATE_DEST_FOLDER/source/$LIBCURLIMPERSONATE_RELEASE/config.log && exit 1)
echo "libcurl successful build log:"
cat_slower $LIBCURLIMPERSONATE_DEST_FOLDER/source/$LIBCURLIMPERSONATE_RELEASE/config.log

export LIBCURL_BUILD_FOLDER=$LIBCURLIMPERSONATE_DEST_FOLDER/build/$LIBCURLIMPERSONATE_RELEASE
ls -al $LIBCURL_BUILD_FOLDER/lib
export PATH=$LIBCURLIMPERSONATE_DEST_FOLDER/build/$LIBCURLIMPERSONATE_RELEASE/bin:$PATH
export LIBCURLIMPERSONATE_RELEASE=$LIBCURLIMPERSONATE_RELEASE

# override existing vars for the rest of the script
export LATEST_LIBCURL_RELEASE=$LIBCURLIMPERSONATE_RELEASE
