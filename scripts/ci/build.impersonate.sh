...
./scripts/ci/build-libcurl-impersonate.sh $LIBCURLIMPERSONATE_RELEASE $LIBCURLIMPERSONATE_DEST_FOLDER $LIBCURL_RELEASE || (echo "libcurl failed build log:" && cat_slower $LIBCURLIMPERSONATE_DEST_FOLDER/source/$LIBCURLIMPERSONATE_RELEASE/config.log && exit 1)
echo "libcurl successful build log:"
